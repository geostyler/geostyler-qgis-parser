import { isGeoStylerFunction } from 'geostyler-style';
 ...
 */
export class QGISStyleParser {
    cqlParser = new CqlParser();
    /**
     * The name of the QGIS Style Parser.
     */
    static title = 'QGIS Style Parser';
    ...
    readStyle(qmlString) {
        return new Promise(resolve => {
            const options = {};
            try {
                parseString(qmlString, options, (err, result) => {
                    if (err) {
                        resolve({
                            errors: [err]
                        });
                    }
                    const geoStylerStyle = this.qmlObjectToGeoStylerStyle(result);
                    resolve({
     ...
     * @param qmlLabel
     */
    parseLabelTemplates(qmlLabel) {
        let geostylerLabel = '';
        const qmlLabelArray = qmlLabel.split('||');
        qmlLabelArray.forEach((part) => {
     ...
     * @param qmlSymbolizer
     */
    qmlSymbolizerLayerPropsToObject(qmlSymbolizer) {
        const qmlMarkerProps = {};
        qmlSymbolizer.prop.forEach((prop) => {
            const key = prop.$.k;
            const value = prop.$.v;
     ...
     */
    qmlObjectToGeoStylerStyle(qmlObject) {
     ...
     * @param opacity
     */
    qmlColorFromHexAndOpacity(hex, opacity) {
     ...
     */
    getRulesFromQmlObject(qmlObject) {
        const qmlRenderer = get(qmlObject, 'qgis.renderer-v2.[0]');
        const qmlRules = get(qmlRenderer, 'rules[0].rule');
        const qmlCategories = get(qmlRenderer, 'categories[0].category');
        const qmlRanges = get(qmlRenderer, 'ranges[0].range');
        ...
        if (Array.isArray(qmlRules) && qmlRules.length > 0) {
            qmlRules.forEach((qmlRule, index) => {
                const filter = this.getFilterFromQmlRule(qmlRule);
                const scaleDenominator = this.getScaleDenominatorFromRule(qmlRule);
            ...
            qmlCategories.forEach((qmlCategory, index) => {
            ...
            const attribute = get(qmlObject, 'qgis.renderer-v2.[0].$.attr');
            qmlRanges.forEach((qmlRange, index) => {
     ...
     */
    getFilterFromQmlRule(qmlRule) {
        const qmlFilter = get(qmlRule, '$.filter');
        if (qmlFilter) {
            return this.cqlParser.read(qmlFilter);
        }
        return undefined;
    }
    /**
     ...
     */
    getScaleDenominatorFromRule(qmlRule) {
     ...
     * @param settings
     */
    getTextSymbolizerFromLabelSettings(settings) {
        let textSymbolizer = {
            kind: 'Text',
        };
     ...
     *
     */
    parseQmlSymbolizers(qmlSymbolizers) {
        const symbolizerMap = {};
        qmlSymbolizers.forEach((qmlSymbolizer) => {
     ...
     */
    getPointSymbolizersFromQmlSymbolizer(qmlSymbolizer) {
        return qmlSymbolizer.layer.map((symbolizerLayer) => {
     ...
     */
    getPointSymbolizerFromMarkLayer(symbolizerLayer) {
        let markSymbolizer = {
            kind: 'Mark',
        };
        const qmlMarkerProps = this.qmlSymbolizerLayerPropsToObject(symbolizerLayer);
     ...
     */
    getLineSymbolizersFromQmlSymbolizer(qmlSymbolizer) {
        const { qmlSymbolizerLayerPropsToObject } = this;
        return qmlSymbolizer.layer.map((symbolizerLayer) => {
            let lineSymbolizer = {
                kind: 'Line',
            };
            const qmlMarkerProps = qmlSymbolizerLayerPropsToObject(symbolizerLayer);
     ...
     */
    getFillSymbolizerFromQmlSymbolizer(qmlSymbolizer) {
        const { qmlSymbolizerLayerPropsToObject } = this;
        return qmlSymbolizer.layer.map((symbolizerLayer) => {
            let fillSymbolizer = {
                kind: 'Fill',
     ...
     */
    getPointSymbolizerFromSvgLayer(symbolizerLayer) {
        let iconSymbolizer = {
            kind: 'Icon',
        };
        const qmlMarkerProps = this.qmlSymbolizerLayerPropsToObject(symbolizerLayer);
     ...
     * @param filter
     */
    getQmlFilterFromFilter(filter) {
        return this.cqlParser.write(filter);
    }
    /**
     *
     */
    getQmlRuleFromRule(rule, index) {
     ...
     */
    getQmlSymbolsFromStyle(geostylerStyle, rules) {
     ...
     * @param rule
     */
    getQmlSymbolFromRule(rule, index) {
     ...
     *
     */
    getQmlLineSymbolFromSymbolizer(symbolizer) {
        let lineColor;
    ...
    getQmlFillSymbolFromSymbolizer(symbolizer) {
     ...
     * @param symbolizer
     */
    getQmlLayerFromSymbolizer(symbolizer) {
    ...
    getQmlMarkSymbolFromIconSymbolizer(symbolizer) {
        let color;
        if (!isGeoStylerFunction(symbolizer.color) && !isGeoStylerFunction(symbolizer.opacity)) {
     ...
     *
     */
    getQmlMarkSymbolFromSymbolizer(symbolizer) {
        let color;
        if (!isGeoStylerFunction(symbolizer.color) && !isGeoStylerFunction(symbolizer.opacity)) {
            color = this.qmlColorFromHexAndOpacity(symbolizer.color, symbolizer.opacity);
        }
     ...
     * @param properties
     */
    propsObjectToQmlSymbolProps(properties) {
     ...
     */
    geoStylerStyleToQmlObject(geoStylerStyle) {
        const type = 'RuleRenderer';
        const rules = [];
        const symbols = this.getQmlSymbolsFromStyle(geoStylerStyle, rules);
    ...
    convertTextSymbolizerRule(qmlRuleList, rule) {
        let textSymbolizer;
    ...
    convertTextSymbolizers(qmlObject, geoStylerStyle) {
        const textSymbolizerRules = [];
        ...
        if (textSymbolizerRules.length > 0) {
            ...
            textSymbolizerRules.forEach(rule => this.convertTextSymbolizerRule(qmlObject.qgis.labeling[0].rules[0].rule, rule));
        }
    }
}
